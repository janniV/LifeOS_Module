...
public final class ScrobblerModule implements LifeModule {
    ...
    public Node getMainView() {
        if (view == null) {
            view = new ScrobblerView(settings, new ScrobblerView.Controller() {
                ...
                @Override public void onClassifyArtist(String artist, ArtistClassificationRule.Type type) {
                    if (artist == null || artist.isBlank()) return;
                    if (settings.artistClassificationRules == null)
                        settings.artistClassificationRules = new ArrayList<>();
                    // De-dupe: replace any existing rule for the same artist substring.
                    String key = artist.trim().toLowerCase();
                    settings.artistClassificationRules.removeIf(
                        r -> r != null && r.matchArtist != null
                             && r.matchArtist.trim().toLowerCase().equals(key));
                    settings.artistClassificationRules.add(new ArtistClassificationRule(artist.trim(), type));
                    settings.save(settingsFile);
                    ScrobblerLog.info("Artist-Regel gespeichert: \"" + artist + "\" -> " + type);
                }
                @Override public void onRemoveArtistClassificationRule(ArtistClassificationRule rule) {
                    if (rule == null || settings.artistClassificationRules == null) return;
                    settings.artistClassificationRules.remove(rule);
                    settings.save(settingsFile);
                    ScrobblerLog.info("Artist-Regel entfernt: \"" + rule.matchArtist + "\"");
                }
                @Override public java.util.List<ArtistClassificationRule> getArtistClassificationRules() {
                    if (settings.artistClassificationRules == null) return java.util.List.of();
                    return Collections.unmodifiableList(settings.artistClassificationRules);
                }
                @Override public int getTrackRewriteCount() {
                    if (settings.trackRewrites == null) return 0;
                    return settings.trackRewrites.size();
                }
                @Override public void onClearTrackRewrites() {
                    if (settings.trackRewrites != null) settings.trackRewrites.clear();
                    settings.save(settingsFile);
                    ScrobblerLog.info("Alle Track-Rewrite-Regeln geloescht");
                }
            });
    ...
    public ModuleAIContext getAIContext() {
        return new ModuleAIContext(
            "Du bist ein Assistent fuer das LifeOS-Scrobbler-Modul. Du kannst die laufende Wiedergabe"
            + " beschreiben, kuerzliche Scrobbles auflisten, das Scrobbeln ein- und ausschalten und"
            + " die Top-Kuenstler:innen der laufenden Sitzung melden.",
            null,
            List.of(
                ...
                new AITool("scrobbler_top_artists",
                    "Top-Kuenstler:innen der laufenden Sitzung. Eingabe: optional eine Zahl (Standard 5).",
    ...
    private void backfillExistingScrobbles(ScrobbleEvent original, NowPlaying corrected) {
        BACKFILL_POOL.execute(() -> {
            try {
                ...
                ScrobblerLog.info("Backfill fuer " + original.artist + " - " + original.title
