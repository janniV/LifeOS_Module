...
import de.janniv.lifeos.scrobbler.ArtistClassificationRule;
 ...
 * History-Tabelle und Einstellungen. Geschaeftslogik liegt komplett in der
 * Engine -- die View hoert nur per Listener mit und schreibt Settings ueber
 * den uebergebenen Consumer zurueck.
 */
public final class ScrobblerView extends BorderPane implements ScrobbleEngine.Listener {
    ...
    public interface Controller {
        ...
        /** Adds or updates an artist-level classification rule. */
        default void onClassifyArtist(String artist, ArtistClassificationRule.Type type) {}
        /** Removes one artist-level classification rule. */
        default void onRemoveArtistClassificationRule(ArtistClassificationRule rule) {}
        /** Returns all currently persisted artist-classification rules. */
        default java.util.List<ArtistClassificationRule> getArtistClassificationRules() {
            return java.util.List.of();
        }
        /** Number of pinned track-rewrite corrections currently stored. */
        default int getTrackRewriteCount() { return 0; }
        /** Deletes all pinned track-rewrite corrections. */
        default void onClearTrackRewrites() {}
    }
    ...
    private final Label nowArtist = new Label("Nichts laeuft");
    ...
    private final ObservableList<ArtistClassificationRule> artistRules = FXCollections.observableArrayList();
    private final ListView<ArtistClassificationRule> artistRuleList = new ListView<>(artistRules);
    private final Label rewriteCountLabel = new Label();
    ...
    private final CheckBox smartParsing = new CheckBox("Smarte Kuenstler:innen-Erkennung fuer Browser-/YouTube-Titel");
    ...
    public ScrobblerView(ScrobblerSettings settings, Controller controller) {
        ...
        // onScrobble; doppelte Eintraege werden anhand des Zeitstempels
        // dedupliziert.
    ...
    private Tab buildPodcastsTab() {
        ...
        TableColumn<PodcastLog.Entry, String> dCol = new TableColumn<>("Laenge");
        ...
        MenuItem delete = new MenuItem("Eintrag loeschen");
        ...
        MenuItem markPodcast = new MenuItem("Interpret immer als Podcast markieren");
        markPodcast.setOnAction(e -> {
            PodcastLog.Entry sel = podcastTable.getSelectionModel().getSelectedItem();
            if (sel != null && !sel.show.isBlank()) {
                controller.onClassifyArtist(sel.show, ArtistClassificationRule.Type.PODCAST);
                refreshArtistRules();
                statusLabel.setText("Regel gespeichert: \"" + sel.show + "\" -> immer Podcast");
            }
        });
        menu.getItems().addAll(edit, new SeparatorMenuItem(), markPodcast,
            new SeparatorMenuItem(), delete, refresh);
        ...
        Label hint = new Label("Diese Eintraege werden nicht zu Maloja geschickt -- nur lokal gespeichert.");
    ...
    private void showPodcastEditDialog(int index, PodcastLog.Entry original) {
        ...
        g.add(new Label("Laenge (Sek.)"), 0, 3); g.add(length, 1, 3);
    ...
    private Tab buildHistoryTab() {
        ...
        tCol.setPrefWidth(72);
        TableColumn<ScrobbleEvent, String> aCol = new TableColumn<>("Kuenstler:in");
        ...
        historyTable.setRowFactory(tv -> {
            ...
            MenuItem deleteOneItem = new MenuItem("Aus Maloja loeschen");
            ...
            MenuItem deleteAllSelectedItem = new MenuItem("Alle markierten loeschen");
            deleteAllSelectedItem.setOnAction(e -> bulkDelete(
                new ArrayList<>(historyTable.getSelectionModel().getSelectedItems())));

            MenuItem markAsPodcast = new MenuItem("Interpret immer als Podcast markieren");
            markAsPodcast.setOnAction(e -> {
                ScrobbleEvent ev = row.getItem();
                if (ev != null && !ev.artist.isBlank()) {
                    controller.onClassifyArtist(ev.artist, ArtistClassificationRule.Type.PODCAST);
                    refreshArtistRules();
                    statusLabel.setText("Regel gespeichert: \"" + ev.artist + "\" -> immer Podcast");
                }
            });
            MenuItem markAsMusic = new MenuItem("Interpret immer als Musik markieren");
            markAsMusic.setOnAction(e -> {
                ScrobbleEvent ev = row.getItem();
                if (ev != null && !ev.artist.isBlank()) {
                    controller.onClassifyArtist(ev.artist, ArtistClassificationRule.Type.MUSIC);
                    refreshArtistRules();
                    statusLabel.setText("Regel gespeichert: \"" + ev.artist + "\" -> immer Musik");
                }
            });

            rowMenu.getItems().addAll(editItem, new SeparatorMenuItem(),
                markAsPodcast, markAsMusic, new SeparatorMenuItem(),
                deleteOneItem, deleteAllSelectedItem);
        ...
        refresh.setOnAction(e -> {
            ...
            controller.loadHistoryFromServer(events -> {
                ...
                Platform.runLater(() -> statusLabel.setText("Verlauf: "
                    + (events == null ? 0 : events.size()) + " Eintraege aus Maloja"));
            });
        });

        Button deleteSelected = new Button("Auswahl loeschen");
        ...
        Label hint = new Label("Strg+Klick fuer Mehrfachauswahl - Doppelklick = bearbeiten - Rechtsklick = Optionen");
        hint.setStyle("-fx-opacity: 0.65; -fx-font-size: 11px;");
    ...
    private void bulkDelete(List<ScrobbleEvent> targets) {
        ...
        if (targets.isEmpty()) {
            statusLabel.setText("Lokal gespeicherte Podcasts koennen nicht aus Maloja geloescht werden.");
            return;
        }
        ...
        String msg = toDelete.size() == 1
            ...
            : toDelete.size() + " Scrobbles aus Maloja loeschen?";
        Alert confirm = new Alert(Alert.AlertType.CONFIRMATION, msg);
        confirm.setHeaderText("Scrobble" + (toDelete.size() == 1 ? "" : "s") + " loeschen");
        ...
        confirm.showAndWait().ifPresent(b -> {
            ...
            statusLabel.setText(toDelete.size() + " Scrobble(s) zum Loeschen gesendet ...");
        });
    }
    ...
    private void showEditDialog(ScrobbleEvent original) {
        ...
        d.setHeaderText("Korrigiert den Eintrag in Maloja (loescht alt, sendet neu).");
        if (getScene() != null) d.initOwner(getScene().getWindow());
        ...
        length.setEditable(true);
        CheckBox remember = new CheckBox("Korrektur fuer dieses Lied merken (gilt fuer zukuenftige Scrobbles)");
        CheckBox rewritePast = new CheckBox("Auch bestehende Maloja-Eintraege dieses Songs nachtraeglich anpassen");
        ...
        GridPane g = new GridPane();
        g.setHgap(10); g.setVgap(8); g.setPadding(new Insets(12));
        g.add(new Label("Kuenstler:in"), 0, 0); g.add(artist, 1, 0);
        ...
        d.showAndWait().ifPresent(result -> {
            ...
            // naechsten Sync taucht der korrigierte Eintrag wieder auf.
    ...
    private Tab buildManualTab() {
        ...
        g.add(new Label("Kuenstler:in"), 0, 0); g.add(mArtist, 1, 0);
        g.add(new Label("Titel"), 0, 1); g.add(mTitle, 1, 1);
        g.add(new Label("Album"), 0, 2); g.add(mAlbum, 1, 2);
        g.add(new Label("Laenge (Sek.)"), 0, 3); g.add(mLength, 1, 3);
        ...
        submit.setOnAction(e -> {
            ...
            if (a.isEmpty() || t.isEmpty()) {
                hint.setText("Kuenstler:in und Titel sind Pflicht.");
                return;
            }
    ...
    private Tab buildSettingsTab() {
        ...
        malojaKey.setPromptText("Privater API-Key (fuers Scrobbeln)");
        g.add(malojaKey, 1, row++, 2, 1);

        g.add(new Label("Maloja-Admin-Passwort"), 0, row);
        malojaAdminPassword.setPromptText("Login-Passwort der Maloja-Weboberflaeche");
        g.add(malojaAdminPassword, 1, row++, 2, 1);
        Label adminHint = new Label("-> Wird nur fuers Loeschen/Bearbeiten gebraucht. "
            + "Maloja akzeptiert API-Keys nur fuers Scrobbeln; alles andere "
            + "laeuft ueber die Login-Session. Lokal gespeichert, geht nicht ins Netz "
            + "ausser zum Maloja-Server selbst.");
        ...
        Button authBtn = new Button("Loesch-Berechtigung pruefen");
        authBtn.setOnAction(e -> {
            persistFromUi();
            MalojaClient probe = new MalojaClient(settings.malojaUrl, settings.malojaApiKey,
                settings.malojaAdminPassword);
            connTest.setText("Pruefe Admin-Login...");
            new Thread(() -> {
                String r = probe.testDeleteAuth();
                Platform.runLater(() -> connTest.setText(r));
            }, "scrobbler-auth-test").start();
        });
        Button openMalojaBtn = new Button("Maloja-Webinterface oeffnen");
        openMalojaBtn.setOnAction(e -> {
            ...
            try {
                ...
                connTest.setText("Konnte Browser nicht oeffnen: " + ex.getMessage()
        ...
        Label webhookHint = new Label("Aktiviert einen lokalen HTTP-Endpunkt: POST http://localhost:{port}/now-playing "
            ...
            + "Damit kannst du Metadaten aus Quellen pushen, die SMTC/MPRIS nicht befuellen (z.B. NAS-Web-App). "
            + "Neustart nach Aenderung erforderlich.");
        ...
        g.add(new Label("Podcast-Laengen-Cutoff (s)"), 0, row);
        podcastCutoff.setEditable(true);
        Label cutHint = new Label("Tracks laenger als dies werden als Podcast/Video behandelt. 0 = aus.");
        cutHint.setStyle("-fx-opacity: 0.6; -fx-font-size: 11px;");
        g.add(podcastCutoff, 1, row);
        g.add(cutHint, 2, row++);

        g.add(new Label("Podcast-Schluesselwoerter (eines pro Zeile)"), 0, row);
        ...
        g.add(new Label("Interpret-Regeln"), 0, row);
        artistRuleList.setPrefHeight(110);
        artistRuleList.setCellFactory(lv -> new ListCell<>() {
            @Override
            protected void updateItem(ArtistClassificationRule r, boolean empty) {
                super.updateItem(r, empty);
                if (empty || r == null) { setText(null); return; }
                String tag = r.type == ArtistClassificationRule.Type.PODCAST ? "[Podcast]" : "[Musik]";
                setText(tag + "  " + r.matchArtist);
            }
        });
        ContextMenu rulesMenu = new ContextMenu();
        MenuItem removeRuleItem = new MenuItem("Regel entfernen");
        removeRuleItem.setOnAction(e -> {
            ArtistClassificationRule sel = artistRuleList.getSelectionModel().getSelectedItem();
            if (sel != null) {
                controller.onRemoveArtistClassificationRule(sel);
                refreshArtistRules();
            }
        });
        rulesMenu.getItems().add(removeRuleItem);
        artistRuleList.setContextMenu(rulesMenu);
        artistRuleList.setPlaceholder(new Label("Noch keine Regeln gespeichert."));

        Button addPodcastRuleBtn = new Button("+ Podcast-Regel");
        addPodcastRuleBtn.setMinWidth(Region.USE_PREF_SIZE);
        addPodcastRuleBtn.setOnAction(e -> showAddRuleDialog(ArtistClassificationRule.Type.PODCAST));
        Button addMusicRuleBtn = new Button("+ Musik-Regel");
        addMusicRuleBtn.setMinWidth(Region.USE_PREF_SIZE);
        addMusicRuleBtn.setOnAction(e -> showAddRuleDialog(ArtistClassificationRule.Type.MUSIC));
        Button removeRuleBtn = new Button("Entfernen");
        removeRuleBtn.setMinWidth(Region.USE_PREF_SIZE);
        ...
        removeRuleBtn.disableProperty().bind(
            javafx.beans.binding.Bindings.isNull(artistRuleList.getSelectionModel().selectedItemProperty()));

        Label rulesHint = new Label("Podcast-Regel: Interpret wird immer lokal gescrobbelt, egal wie kurz. "
            + "Musik-Regel: Interpret wird nie als Podcast erkannt, egal wie lang.");
        rulesHint.setWrapText(true);
        rulesHint.setStyle("-fx-opacity: 0.6; -fx-font-size: 11px;");
        HBox ruleBtns = new HBox(6, addPodcastRuleBtn, addMusicRuleBtn, removeRuleBtn);
        VBox rulesBox = new VBox(5, artistRuleList, ruleBtns, rulesHint);
        g.add(rulesBox, 1, row++, 2, 1);
        ...
        g.add(new Label("Minimale Tracklaenge (s)"), 0, row);
        ...
        g.add(new Label("Kuenstler:in-Regex-Muster"), 0, row);
        ...
        // Small, low-prominence section for clearing learned corrections.
        // Deliberately not a big button -- clearing these is irreversible and rarely needed.
        updateRewriteCountLabel();
        rewriteCountLabel.setStyle("-fx-opacity: 0.6; -fx-font-size: 11px;");
        Button clearRewrites = new Button("Gelernte Korrekturen loeschen");
        clearRewrites.setStyle("-fx-font-size: 10px; -fx-padding: 2 8 2 8; -fx-opacity: 0.7;");
        clearRewrites.setMinWidth(Region.USE_PREF_SIZE);
        clearRewrites.setOnAction(e -> {
            Alert confirm = new Alert(Alert.AlertType.CONFIRMATION,
                "Alle gespeicherten Titelkorrekturen loeschen?\nDies kann nicht rueckgaengig gemacht werden.");
            confirm.setHeaderText("Gelernte Korrekturen loeschen");
            if (getScene() != null) confirm.initOwner(getScene().getWindow());
            confirm.showAndWait().ifPresent(b -> {
                if (b != ButtonType.OK) return;
                controller.onClearTrackRewrites();
                updateRewriteCountLabel();
            });
        });
        HBox rewriteRow = new HBox(8, rewriteCountLabel, clearRewrites);
        rewriteRow.setAlignment(javafx.geometry.Pos.CENTER_LEFT);
        g.add(rewriteRow, 1, row++, 2, 1);
    ...
    private void refreshArtistRules() {
        Platform.runLater(() -> artistRules.setAll(controller.getArtistClassificationRules()));
    }

    private void updateRewriteCountLabel() {
        int count = controller.getTrackRewriteCount();
        rewriteCountLabel.setText(count == 0
            ? "Keine gespeicherten Korrekturen."
            : count + " gespeicherte Korrektur" + (count == 1 ? "" : "en") + ".");
    }

    private void showAddRuleDialog(ArtistClassificationRule.Type type) {
        String typeLabel = type == ArtistClassificationRule.Type.PODCAST ? "Podcast" : "Musik";
        TextInputDialog dlg = new TextInputDialog();
        dlg.setTitle("Interpret als " + typeLabel + " markieren");
        dlg.setHeaderText("Interpret-Name eingeben (Teilstring genuegt, Gross-/Kleinschreibung egal):");
        dlg.setContentText("Interpret:");
        if (getScene() != null) dlg.initOwner(getScene().getWindow());
        dlg.showAndWait().ifPresent(artist -> {
            artist = artist.trim();
            if (!artist.isEmpty()) {
                controller.onClassifyArtist(artist, type);
                refreshArtistRules();
                statusLabel.setText("Regel gespeichert: \"" + artist + "\" -> immer " + typeLabel);
            }
        });
    }
    ...
    public void onNowPlaying(NowPlaying current, NowPlaying enhanced) {
        Platform.runLater(() -> {
            if (current == null || current.isEmpty()) {
                nowTitle.setText("--");
                nowArtist.setText("Nichts laeuft");
                nowAlbum.setText("");
                nowSource.setText("");
            ...
            nowArtist.setText(display.artist().isEmpty() ? "Unbekannte:r Kuenstler:in" : display.artist());
